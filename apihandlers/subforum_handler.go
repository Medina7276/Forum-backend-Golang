...
func NewSubforumHandler(endpoint string, service *service.SubforumService, post *service.PostService) *SubforumHandler {
...
func (sh *SubforumHandler) CreateSubforum(w http.ResponseWriter, r *http.Request) {
	...
	role := &model.SubforumRole{
		...
		UserID:     user.ID,
		SubforumID: created.ID,
	}
	_, err = sh.SubforumRoleService.Create(role)
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
...
func (sh *SubforumHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	idFromURL := r.URL.Path[len(sh.Endpoint):]
	subforum.ID = uuid.FromStringOrNil(idFromURL)
